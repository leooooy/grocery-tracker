...
> 单位若填 `kg`/`KG`/`千克`/`公斤`，录入时自动换算成「斤」（1kg=2斤：数量×2、单价÷2，
> 花费不变），CSV 里统一存「斤」。例如 `-u kg` 单价 10、数量 1 → 存为 5.00 元/斤 × 2 斤。
...
按提示输入日期/品名/单位/单价/数量/打折/商家/备注。Ctrl+C 退出。

连续录入时，**日期**和**商家**会沿用上一条的值（提示里以 `[…]` 带出），回车即沿用；
中途想换商家或改日期，直接输入新值，后续条目会跟着新值走。退出程序后不保留。
